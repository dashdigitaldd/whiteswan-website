...
document.querySelectorAll('[data-enquire]').forEach(b=>b.addEventListener('click',()=>{if(!dialog||!form)return;if(!origin){if(!dialog.open)dialog.showModal();return;}if(form.hidden){form.reset();form.hidden=false;dialog.querySelector('.enquiry-success').hidden=true;contactSession=null;status.textContent='';}form.elements.service.value=b.dataset.enquire||'stay';if(!dialog.open)dialog.showModal();void getSession('contact').catch(()=>{});void track('enquiry_open',b.dataset.enquire,b.dataset.placement);}));
