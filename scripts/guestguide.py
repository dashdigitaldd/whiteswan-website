...
def guide(lang, head, components):
    ...
    doc = source(lang)
    chapters = [('arrive', T('Arriving', 'Tu llegada')), ('services', T('Little luxuries', 'Pequeños lujos')), ('villa', T('The spaces', 'Los espacios')), ('house', T('At the villa', 'En la villa')), ('rules', T('Good to know', 'Lo esencial')), ('checkout', T('Before you leave', 'Antes de partir')), ('explore', T('Explore the coast', 'Explora la costa')), ('faq', T('FAQs', 'Preguntas frecuentes'))]
    ...
    for key, label in chapters:
        ...
        if key == 'house':
            from hospitality import HOUSE_TOPICS
            for topic, detail in zip(HOUSE_TOPICS, el.xpath('./details')):
                detail.set('id','house-'+topic)
        if key == 'services':
