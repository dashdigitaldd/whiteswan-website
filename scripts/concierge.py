...
def widget(lang,path):
    ...
    return f'''<details class="concierge-launcher"><summary><span class="chat-symbol" aria-hidden="true">✧</span><span>{T('Ask White Swan','Pregunta a White Swan')}<small>WhatsApp</small></span><span class="concierge-toggle" aria-hidden="true">+</span></summary><div class="concierge-panel"><span class="eyebrow">WHITE SWAN CONCIERGE</span><h2>{T('What’s on your mind?','¿En qué te ayudamos?')}</h2><p>{T('Villa questions, local tips or a little something extra. Choose a topic to continue on WhatsApp.','Preguntas sobre la villa, consejos locales o algo especial. Elige un tema para continuar por WhatsApp.')}</p><nav aria-label="{T('WhatsApp enquiry topics','Temas de consulta por WhatsApp')}">{''.join(link(lang,topic,path,'other',classes='concierge-topic') for topic in topics)}</nav><p class="fine">{T('You send the message. You can ask for the host at any time.','Tú envías el mensaje. Puedes pedir hablar con el anfitrión en cualquier momento.')}</p></div></details>'''
